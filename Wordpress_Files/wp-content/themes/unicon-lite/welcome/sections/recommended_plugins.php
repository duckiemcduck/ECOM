<?php
	...
	if(!empty($pro_plugins)) {
		?>
		<h4 class="recomplug-title"><?php echo esc_html__('Pro Plugins', 'unicon-lite'); ?></h4>
		<p><?php echo esc_html__('Take Advantage of some of our Premium Plugins', 'unicon-lite'); ?></p>
		<div class="recomended-plugin-wrap">
		<?php
		foreach($pro_plugins as $plugin) {

			$status = $this->unicon_lite_plugin_active($plugin);
			$icon_url = $plugin['screenshot'];
			switch($status) {
				case 'install' :
					$btn_class = 'install-offline button';
					$label = esc_html__('Install and Activate', 'unicon-lite');
					$link = $plugin['location'];
					break;

				case 'inactive' :
					$btn_class = 'button';
					$label = esc_html__('Deactivate', 'unicon-lite');
					$link = admin_url('plugins.php');
					break;

				case 'active' :
					$btn_class = 'activate-offline button button-primary';
					$label = esc_html__('Activate', 'unicon-lite');
					$link = $plugin['location'];
					break;
			}
	...
	if(!empty($free_plugins)) {
		?>
		<h4 class="recomplug-title"><?php echo esc_html__('Free Plugins', 'unicon-lite'); ?></h4>
		<p><?php echo esc_html__('These Free Plugins might be handy for you.', 'unicon-lite'); ?></p>
		<div class="recomended-plugin-wrap">
		<?php
		foreach($free_plugins as $plugin) {
			$info = $this->unicon_lite_call_plugin_api($plugin['slug']);

			$icon_url = $this->unicon_lite_check_for_icon($info->icons);
			$status = $this->unicon_lite_plugin_active($plugin);
			$btn_url = $this->unicon_lite_plugin_generate_url($status, $plugin);
